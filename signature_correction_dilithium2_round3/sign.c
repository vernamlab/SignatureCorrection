...
int crypto_sign_verify_signature_correction(const uint8_t *sig,
                       size_t siglen,
                       const uint8_t *m,
                       size_t mlen,
                       const uint8_t *pk)
{
  ...
  FILE *recovered_bits_file;
  recovered_bits_file = fopen("recovered_bits.txt", "a");

  for(int bit_index = 0; bit_index < 32; ++bit_index) {
    int multiplier = 1;
    int breaker = 0;
    printf("\nBitflip at index %d?", bit_index);
    printf("\nBitflip at index %d?", bit_index);
    for(int mul = 0; mul < bit_index; mul++) {
      multiplier = multiplier * 2;
    }

    for(int row = 0; row < L; ++row) {

      // Checking for 1->0 flips
      for(int col = 0; col < N; ++col) {
        ...
        if (col == 0) {   // Copying cp in 1st iteration and changing my_c afterwards
          my_c = cp;
        }
        ...
        for(int k = 0; k < N; k++) {
          int32_t u = my_c.coeffs[k];
          if(u > (Q-1)/2) u -= Q;   // Converting to +1 and -1
          if(u < -(Q-1)/2) u += Q;  // Converting to +1 and -1
          z.vec[row].coeffs[k] = z.vec[row].coeffs[k] + multiplier * u;
          // If a coeff turns negative, following must be done to make it positive, freeze does not do that.
          if ((int32_t)z.vec[row].coeffs[k] < 0)
            z.vec[row].coeffs[k] = z.vec[row].coeffs[k] + Q;
        }

        poly_reduce(&z.vec[row]);
        ...
        if (verified == 1) {
          printf("\n\nYes, correction successful 👍\n");
          printf("\ns1(%d,%d)_%d = %d\n", row, col%N, bit_index, 1);
          
          // Writing recovered bits to file
          fprintf(recovered_bits_file, "%d, %d, %d, %d\n", row, col%N, bit_index, 1);
          ...
          // Shift c right
          uint32_t temp = my_c.coeffs[N-1];
          for (int shift = N-1; shift > 0; shift--) {
            my_c.coeffs[shift] = my_c.coeffs[shift-1];
          }
          my_c.coeffs[0] = Q-temp;     // IMPORTANT, rotated value of c must be flipped from -1 to 1 and vice versa.
        }
      }
    ...
    // Stop searching for further bitflip indexes
    if (breaker == 1)
        break;
  }
  fclose(recovered_bits_file);
  printf("\n");
  return 0;
}

int crypto_sign_open_signature_correction(uint8_t *m,
                     size_t *mlen,
                     const uint8_t *sm,
                     size_t smlen,
                     const uint8_t *pk)
{
  ...
  *mlen = smlen - CRYPTO_BYTES;
  if(crypto_sign_verify_signature_correction(sm, CRYPTO_BYTES, sm + CRYPTO_BYTES, *mlen, pk))
    goto badsig;
  else {
    /* All good, copy msg, return 0 */
    for(i = 0; i < *mlen; ++i)
